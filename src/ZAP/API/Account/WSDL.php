...
/**
 * ZAP_API_Account_Soap is a class which allows to manage Zimbra accounts via SOAP
 * @package   ZAP
 * @category  Account
...
class ZAP_API_Account_WSDL extends ZAP_API_Account_Base
{
	...
	public function changePassword($oldPassword, $password, $virtualHost = '')
	{
		...
		$result = $this->_client->changePasswordRequest($params);
		$authToken = $result->authToken;
		if($authToken) $this->_client->authToken($authToken);
		return $result;
	}
	...
	public function autoCompleteGal($name){
		...
		return $this->_client->autoCompleteGalRequest($attributes);
	}
	...
	public function checkLicense($feature)
	{
		...
		return $this->_client->checkLicenseRequest($params);
	}
	...
	public function checkRights($type, $key, array $rights)
	{
		...
		return $this->_client->checkRightsRequest($params);
	}
	...
	public function createIdentity($name, array $attrs = array()){
		...
		return $this->_client->createIdentityRequest($params);
	}
	...
	public function createSignature($name, $content = '', $type = 'text/plain')
	{
		...
		return $this->_client->createSignatureRequest($params);
	}
	...
	public function deleteIdentity($name)
	{
		...
		return $this->_client->deleteIdentityRequest($params);
	}
	...
	public function deleteSignature($name)
	{
		...
		return $this->_client->deleteSignatureRequest($params);
	}
	...
	public function discoverRights(array $rights)
	{
		...
		return $this->_client->discoverRightsRequest($params);
	}
	...
	public function distributionListAction($name, array $action, array $attrs = array())
	{
		...
		return $this->_client->distributionListActionRequest($params);
	}
	...
	{
		return $this->_client->endSessionRequest();
	}
	...
	public function getAccountDistributionLists($ownerOf = FALSE, $memberOf = 'directOnly', array $attrs = array())
	{
		...
		return $this->_client->getAccountDistributionListsRequest($params);
	}
	...
	{		
		return $this->_client->getAvailableCsvFormatsRequest();
	}
	...
	{
		return $this->_client->getAvailableLocalesRequest();
	}
	...
	public function getDistributionList($name, array $attrs = array(), $needOwners = FALSE, $needRights = '')
	{
		...
		if(!empty($needRights)) $params['$needRights'] = $needRights;
		if(count($attrs))
		{
			$attributes = array();
			foreach ($attrs as $attr)
	...
	public function getDistributionListMembers($name, $limit = 0, $offset = 0)
	{
		...
		return $this->_client->getDistributionListMembersRequest($params);
	}
	...
	{
		return $this->_client->getIdentitiesRequest();
	}
	...
	public function getInfo(array $sections = array(), array $rights = array())
	{
		...
		{
			$params['sections'] = $sectionsStr;
		}

		$rightsStr = $this->_commaAttributes($rights);
		if(!empty($rightsStr))
		{
			$params['rights'] = $rightsStr;
		}
		return $this->_client->getInfoRequest($params);
	}
	...
	public function getPrefs(array $prefs = array())
	{
		...
		return $this->_client->getPrefsRequest($params);
	}
	...
	public function getRights(array $ace = array())
	{
		...
		return $this->_client->getRightsRequest($params);
	}
	...
	public function getSMIMEPublicCerts(array $stores, array $emails = array())
	{
		...
		$value = (isset($stores['_']) AND in_array($stores['_'], $lookupOpts)) ? $stores['_'] : 'GAL';
		$params = array(
			'store' => array(
		...
		return $this->_client->getSMIMEPublicCertsRequest($params);
	}
	...
	public function getShareInfo($owner = '', array $grantee = array(), $internal = TRUE, $includeSelf = TRUE)
	{
		...
		return $this->_client->getShareInfoRequest($params, $options);
	}
	...
	{
		return $this->_client->getSignaturesRequest();
	}
	...
	{
		return $this->_client->getWhiteBlackListRequest();
	}
	...
	public function grantRights(array $ace)
	{
		...
		return $this->_client->grantRightsRequest($params);
	}
	...
	public function modifyIdentity($name, array $attrs = array())
	{
		...
		return $this->_client->modifyIdentityRequest($params);
	}
	...
	public function modifyPrefs(array $prefs = array())
	{
		...
		return $this->_client->modifyPrefsRequest($params);
	}
	...
	public function modifyProperties($zimlet, $prop_name, $value)
	{
		...
		return $this->_client->modifyPropertiesRequest($params);
	}
	...
	public function modifySignature($name, $content = '', $type = 'text/plain')
	{
		...
		return $this->_client->modifySignatureRequest($params);
	}
	 ...
	 * @param  array  $filter Search filter specification
	 * @param  array  $attrs  Search attributes
	 * @return mixed
	 */
	...
	{
		$params = array(
			'op' => ((bool) $subscribe) ? 'subscribe' : 'unsubscribe',
			'dl' => array(
				'by' => 'name',
				'_' => $name,
			),
		);
		return $this->_client->subscribeDistributionListRequest($params);
	}
	...
	public function syncGal($token, $galAcctId = '')
	{
		...
		return $this->_client->syncGalRequest($params);
	}
}